...
function MedicationRow({ medication, index, onChange, onRemove, canRemove }) {
  ...
  const field = (key, value) => onChange(index, { ...medication, [key]: value });
    ...
    <div className="space-y-2.5">
      {checkedMedications.map((med, i) =>
        med.quickCheckMessage ? (
          <div key={i} className="border border-amber-300 bg-amber-50 rounded-lg p-3">
...
export default function PrescriptionModal({
  ...
  useEffect(() => {
    ...
    if (existingPrescription?.medications?.length) {
      setMedications(existingPrescription.medications.map(medicationToFormState));
    } else {
  ...
  const runSafetyCheck = useCallback(async (meds) => {
        ...
        <div className="p-5 sm:p-6 grid grid-cols-1 lg:grid-cols-3 gap-5">
          ...
          <div className="space-y-3">
            <div className="bg-white rounded-xl shadow overflow-hidden border border-gray-100">
              ...
              <div className="p-4">
                <SafetyPanel loading={checkingSafety} checkedMedications={checkedMedications} />
              </div>
            </div>
          </div>
        </div>
