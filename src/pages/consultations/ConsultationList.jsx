...
function ConsultationsCalendar({ selectedDate, onSelectDate, dateCounts }) {
  ...
  return (
    <div className="mini-calendar-card mb-6">
...
const Consultations = () => {
  ...
  useEffect(() => {
    ...
    const result = consultations.filter((c) => {
      ...
      const matchesDate =
        !selectedDate || toDateKey(c.createdAt) === toDateKey(selectedDate);

      return matchesQuery && matchesDate;
    });

    setFiltered(result);
  }, [search, selectedDate, consultations]);
  ...
  const getUrgencyBadge = (level) => {
    const styles = {
      ...
      critical:
        "bg-rose-50 text-rose-600 border-rose-200/60 animate-pulse font-bold",
    };
    return (
          ...
          <div>
                      ...
                      >
                        ...
                        <td className="px-6 py-4 capitalize font-bold">
                          <span className="inline-flex items-center gap-2 text-slate-700 text-xs">
                            <span
                              className={`w-2.5 h-2.5 rounded-full ring-4 shadow-sm ${c.status?.toLowerCase() === "completed" ? "bg-emerald-500 ring-emerald-100" : "bg-amber-400 ring-amber-100"}`}
                            ></span>
                            {c.status}
                          </span>
                        </td>
            ...
            <div className="block md:hidden space-y-4">
              {paginatedData.map((c) => (
                <div
                  key={c._id}
                  className="bg-white rounded-2xl p-5 shadow-md border border-slate-100 space-y-4"
                >
                  <div className="flex items-start justify-between gap-2 border-b border-slate-50 pb-3">
                    <div>
                      ...
                      <div className="flex items-center gap-1.5 mt-1">
                        <span
                          className={`w-2 h-2 rounded-full ${c.status?.toLowerCase() === "completed" ? "bg-emerald-500" : "bg-amber-400"}`}
                        ></span>
                    ...
                    <span
                      className={`px-2.5 py-1 rounded-xl text-xs font-bold border capitalize tracking-wide shadow-sm shrink-0 ${getUrgencyBadge(c.urgencyLevel)}`}
                    >
                      {c.urgencyLevel}
                    </span>
                  </div>
                  ...
                  <div className="flex gap-2 pt-2 border-t border-slate-50">
                    <Link
                      to={`/consultations/${c._id}`}
                      className="flex-1 inline-flex items-center justify-center bg-slate-100 text-slate-700 py-2.5 rounded-xl text-xs font-bold border border-slate-200 transition gap-1"
                    >
                      <svg
                        className="w-4 h-4"
                        fill="none"
