...
const PatientSearch = () => {
  ...
  useEffect(() => {
    const timeout = setTimeout(() => { loadPatients(search); }, 400);
    ...
    <div className="p-6 max-w-4xl mx-auto min-h-screen bg-gradient-to-tr from-slate-50 via-gray-50 to-blue-50/30 antialiased text-slate-800">
      ...
      <div className="mb-8">
        <h2 className="text-3xl font-extrabold text-blue-600 tracking-tight mb-2">
          {t('consultations.searchPatient')}<span className="text-blue-700">.</span>
        </h2>
        <p className="text-slate-500 text-sm font-medium">{t('consultations.searchSubtitle')}</p>
      </div>
      ...
      {loading && (
        <div className="flex flex-col items-center justify-center py-20 space-y-4">
          ...
          <p className="text-sm font-semibold text-slate-400 tracking-wide animate-pulse">{t('consultations.searchingPatients')}</p>
        </div>
      )}
      ...
      {!loading && patients.length === 0 && (
        <div className="text-center py-20 bg-white/70 backdrop-blur-sm border border-dashed border-slate-200 rounded-2xl shadow-sm">
          ...
          <p className="text-slate-600 font-bold text-lg">{t('consultations.noPatientsFound')}</p>
          <p className="text-slate-400 text-sm mt-1">{t('consultations.noPatientsHint')}</p>
        </div>
      )}
          ...
          {patients.map((p) => (
            ...
            >
              <div className="flex items-center gap-4">
                ...
                <div>
                  <p className="font-bold text-slate-900 group-hover:text-blue-600 transition-colors duration-200 text-base">{p.name}</p>
                  <div className="flex flex-wrap items-center gap-x-2.5 gap-y-1.5 text-xs mt-2 font-medium">
                    ...
                    <span className="bg-slate-100 text-slate-600 px-2.5 py-1 rounded-lg border border-slate-200/40">
                      {t('consultations.yrs') !== 'Yrs' ? `${calculateAge(p.dateOfBirth)} ${t('consultations.yrs')}` : `Age: ${calculateAge(p.dateOfBirth)} Yrs`}
                    </span>
                    <span className={`px-2.5 py-1 rounded-lg border uppercase ${p.gender?.toLowerCase() === 'male' ? 'bg-blue-50/60 text-blue-600 border-blue-100' : 'bg-rose-50/60 text-rose-600 border-rose-100'}`}>
                      {p.gender || 'N/A'}
                    </span>
                  </div>
                </div>
