...
export default function Settings() {
  ...
  const handleSave = async (e) => {
    ...
    if (!confirmEmail.trim()) {
      setError('Please confirm your account email to save changes.');
      return;
    }
  ...
  return (
    <div className="max-w-2xl mx-auto px-6 py-10">
      <div className="bg-white border border-gray-200 rounded-xl p-8">
        ...
        {error && (
          <div className="bg-red-50 text-red-600 text-sm rounded-md p-3 mb-5">{error}</div>
        )}
        {success && (
          <div className="bg-green-50 text-green-700 text-sm rounded-md p-3 mb-5">{success}</div>
        )}
        ...
        <form onSubmit={handleSave} className="space-y-5">
          ...
          <div className="border-t border-gray-100 pt-5">
            <label className="block text-sm text-gray-600 mb-1">
              Confirm your account email to save changes
            </label>
            <input
              type="email"
              value={confirmEmail}
